    ...
    for (int i = 0; i < roomCount; i++) {
        if (hotelRooms[i].isOccupied) {
            printf("Room Number: %d, Type: %s, Floor: %d, View: %s, Customer: %s\n, Stayed: %d",
                   ...
                   hotelRooms[i].view, hotelRooms[i].customerName, hotelRooms[i].stayDuration);
        }
    }
    // for (int i = 0; i < roomCount; i++) {
    //     if (hotelRooms[i].roomNumber == roomNumber && !hotelRooms[i].isOccupied && strcmp(hotelRooms[i].type, type) == 0 && hotelRooms[i].floor == floor) {
...
int login() {
    ...
    ((11<month &&  month<12 ) || (1<month &&  month<4 ))? 
    printf("    Welcomee to Sea villah hotel. happy winter!   \n"):
    printf("    Welcomee to Sea villah hotel. happy Summer!   \n");
