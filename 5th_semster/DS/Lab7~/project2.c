...
void process_baseline(Order o, bool debug) {
    if (debug) printf("[BASELINE] %s Order #%d (Qty: %d @ $%d)\n", side_str(o.side), o.id, o.qty, o.price);
    ...
    while (o.qty > 0) {
        ...
        if (best_idx != -1) {
            ...
            if (debug) printf("  -> MATCHED %d units with Order #%d @ $%d\n", trade_qty, match->id, match->price);
    ...
    if (o.qty == 0 && debug) {
        printf("  -> Order fully filled.\n");
    }

    if (o.qty > 0) {
        if (debug) printf("  -> Added remaining %d units to %s book.\n", o.qty, side_str(o.side));
        add_to_baseline(o);
    }
}
...
void process_optimized(Order o, bool debug) {
    if (debug) printf("[OPTIMIZED] %s Order #%d (Qty: %d @ $%d)\n", side_str(o.side), o.id, o.qty, o.price);
    ...
    while (node != TNULL && o.qty > 0) {
        ...
        while (node->head != NULL && o.qty > 0) {
            ...
            if (debug) printf("  -> MATCHED %d units with Order #%d @ $%d\n", trade_qty, curr->order.id, node->price);
...
void print_optimized_book() {
    ...
    printf("BUY BOOK (Bids - Descending)\n");
    printf("  ID    | QTY   | PRICE\n");
    printf("  ------------------------\n");
    print_tree_desc(opt_buy_tree.root);
    printf("\nSELL BOOK (Asks - Ascending)\n");
    printf("  ID    | QTY   | PRICE\n");
    printf("  ------------------------\n");
